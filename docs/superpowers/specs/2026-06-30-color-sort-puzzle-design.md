...
The puzzle uses six transparent bottle-like columns with eight vertical color slots each. Initial boards use a random-looking distribution of four colors across all six bottles, not fixed empty helper bottles. Players click or tap one bottle, then another bottle to pour the top contiguous color group when the move is legal. The puzzle is solved when every non-empty bottle contains a single color and is full.

A star control can load a harder optional board using five colors across six bottles. Generated boards are checked with the normal color-sort rules before adoption, with star mode using a deeper bounded solver check than normal mode. The UI marks star mode with an active star button and a visible star-mode badge.
