...
**Goal:** Homeの「今週の数学」見出しをApple系ブルーにし、問題カード内タイトルを一段小さくして情報階層を整える。
...
- セクション見出しはライトテーマ `#0071E3`、ダークテーマ `#5CABFF` とする。
...
- Produces: ライト `text-[#0071E3]`／ダーク `dark:text-[#5CABFF]` のセクション見出しと、`text-[22px] sm:text-[28px]` の問題カードタイトル。
...
Expected: `今週の数学` に `text-[#0071E3]`／`dark:text-[#5CABFF]` がなく、カードタイトルに `text-[22px]`／`sm:text-[28px]` がないためFAILする。
