...
## 2. OpenAI fact plannerの利用条件を再確認する
...
AWS・Amplify反映の明示承認後、本番Originから次の質問をexactly 1回送信します。この質問は高確信の決定論経路なので、正常時はOpenAI APIを呼びません。
...
未回答質問の本文・history・閲覧pathは保存しません。既存の `tti-ai-assistant-unanswered` tableはCloudFormationから安全に外すまで保持しますが、Assistant Lambdaへtable名を渡さず、書き込みIAM権限も付与しません。過去に保存されたitemの棚卸し・削除は、対象と復旧可否を確認した別承認の操作として扱います。

お知らせ・掲示板・数学などの動的コンテンツはLambda内で検索し、本文・抜粋・タイトルをOpenAIへ送りません。利用者へ返す動的URLは、種別ごとの同一サイトpath検証を通したものだけにします。
...
- low-confidence fact plannerの呼び出し数、OpenAI残高と実測cost

既定値は日次200回、sessionあたり10分間に20回、API Gateway stage throttleはrate 2・burst 4です。`ASSISTANT_DAILY_LIMIT=200`を変更する前に、planner呼び出し率、1回あたりの実測token、実測costを確認して再承認を得ます。高確信の質問も利用枠は1回消費しますが、OpenAIは呼びません。
...
- OpenAI fact plannerを使う曖昧質問のlive smoke
